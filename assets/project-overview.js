            ...
            const sortedRows = Array.from(tbody.querySelectorAll("tr")).sort((a, b) => {
                ...
                const getValue = (cell) => {
                    switch (type) {
                        ...
                        case "date":
                            return new Date(cell.querySelector("input[type='date']")?.value || "");
                        case "user":
                            return cell.dataset.selectedName?.toLowerCase() || "";
                        case "select":
                            return cell.querySelector("select")?.selectedOptions[0]?.textContent.trim().toLowerCase() || "";
                        case "text-input":
                            return cell.querySelector("input[type='text']")?.value.toLowerCase() || "";
                        case "number-input":
                            return parseFloat(cell.querySelector("input[type='number']")?.value) || 0;
                        default:
