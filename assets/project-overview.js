...
function initProjectOverviewFilters() {
    ...
    // Filter select2
    const filterSelect = $('#filterSelect').select2({
        ...commonConfig,
    }).on('select2:select', () => {
    ...
    // Columns select2
    const columnSelect = $('#columnSelect').select2(commonConfig).on('change.select2', () => {
...
function initProjectOverviewTable() {
    ...
    $(document).on('click', '.dropdown-item .table-button.status', function () {
        const [ticketId, newStatus, className, name] = $(this).data('args').split(',');
